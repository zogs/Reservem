...
namespace Zogs\ContactBundle\Tests\Controller;
...
class ContactControllerTest extends WebTestCase
{
	...
	public function testContactPage()
	{
		...
		$crawler = $this->client->request('GET',$this->router->generate('contact_us'));
		...
		$this->assertEquals('Zogs\ContactBundle\Controller\ContactController::contactAction',$this->client->getRequest()->attributes->get('_controller'));	
	...
	public function testBotContactPage()
	{
		...
		$data = array(
			...
			'login' => '',
			'_token' => $this->client->getContainer()->get('form.csrf_provider')->generateCsrfToken('contact_us'),);

		//submit the form too fast for human
		$crawler = $this->client->request('POST',$this->router->generate('contact_us'),array('contact_form' =>$data));
